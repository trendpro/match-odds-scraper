...
def parse_html_dump(html_dump):
    ...
    events = soup.find_all('ms-event', class_='grid-event ms-active-highlight ng-star-inserted')

    for event in events:
        event_data = {}

        # Extract tournament name
        tournament_name = event.find_previous('ms-league-header').find('div', class_='title').text.strip()
        ...
        if event_details:
            ...
            if event_name:
                ...
                if '-' in event_name_text:
                    tournament, event_name = map(str.strip, event_name_text.split('-', 1))
            ...
            if event_info:
                timer = event_info.find('ms-prematch-timer', class_='starting-time timer-badge ng-star-inserted')
                if timer:
        ...
        for option in options:
            ...
            if len(odds) == 2:
                player1_odds = odds[0].text.strip().split()[0]  # Extracting the odds from text content
                player2_odds = odds[1].text.strip().split()[0]  # Extracting the odds from text content
